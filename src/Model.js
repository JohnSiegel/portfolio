...
export default function Model({ scroll, ...props }) {
    ...
    const [currentObject, set] = useState()
    const extras = { receiveShadow: true, castShadow: true, "material-envMapIntensity": 0.2 }
    useEffect(() => void (actions["CameraAction.005"].play().paused = true))
    useEffect(() => {
        if (currentObject) group.current.getObjectByName(currentObject).material.color.set("white")
    }, [currentObject])
    useFrame((state) => {
        set(group.current.children[0].children[Math.min(Math.floor(scroll.current * 0.9 * group.current.children[0].children.length), group.current.children[0].children.length)].name);
        ...
        group.current.children[0].children.forEach((child, index) => {
            child.material.color.lerp(color.set(currentObject === child.name ? "tomato" : "#202020").convertSRGBToLinear(), currentObject ? 0.1 : 0.05)
